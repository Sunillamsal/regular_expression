...
function questionOne(){
		...
		var start = /^(\+97798)([0-9]{8})$/; 		// RE that checks number starting with +97798 and eight numbers behind it.
...
function questionTwo(){
		...
		var domainCheck = /((w{3}).)?([a-z0-9\-])+((.com)|(.net)|(.edu))$/; // RE that checks domain name starting either with www. or without it and has name with alphanumeric characters and - which then ends with either .com, .net, .edu.
...
function questionThree(){
		...
		var stringCheck = /((a-zA-Z)|_|-)/;  // RE that checks whether the string consists of A-Z, a-z, - or _.
...
function questionFour(){
		...
		var dateCheck = /^(1[0-2]|[1-9])\/([1-9]|[1-2][0-9]|3[0-2])\/([0-9]{4})$/;	  // RE that checks date with patterns mm/dd/yyyy, m/dd/yyyy, mm/d/yyyy, m/d/yyyy.
...
function questionFive(){
		...
		var timeCheck = /^(2[0-3]|1[0-9]|[0-9]):([0-9]|[1-5][0-9]):([0-9]|[1-5][0-9])$/;	// RE that checks time with patterns hh:mm:ss, h:mm:ss.
...
function questionSix(){
		...
		var hexCheck = /^#([0-9]|[a-f]|[A-F]){1,6}$/;	// RE  that checks hex color with first character starting with # and then consists of hex value of atleast 1 to 6 characters.
